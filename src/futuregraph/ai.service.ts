...
export class AiService {
  ...
  private createSystemPrompt(roundNumber: number, previousRounds: any[]): string {
    const round = RoundDefinitions[roundNumber];
    
    let prompt = `You are an AI therapist assistant implementing the FutureGraph™ Pro+ methodology.
...
${previousRounds.map(r => `Round ${r.roundNumber}: ${JSON.stringify(r.analysis)}`).join('\n')}
` : ''}
